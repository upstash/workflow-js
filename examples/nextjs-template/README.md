...
See the demo at https://workflow-nextjs-template.app.
...
Note that the project won't work in preview. It should be deployed to production like above. This is because preview requires authentication. You can read more about the fix the [troubleshooting page](https://upstash.com/docs/workflow/troubleshooting/vercel).
...
You also have to set the `VERCEL_URL` to your local server url like `http://localhost:3000`. This is required for the app to call the `/api/mock-api` endpoint.
