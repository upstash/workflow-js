...
describe("workflow client", () => {
  ...
  test("should trigger workflow run", async () => {
    ...
    await mockQStashServer({
      ...
      responseFields: {
        status: 200,
        body: [{ workflowRunId: `wfr_${myWorkflowRunId}`, workflowCreatedAt: 1735689600000 }],
      },
      receivesRequest: {
        method: "POST",
        url: `${MOCK_QSTASH_SERVER_URL}/v2/batch/trigger`,
        token,
        body: [
          {
  ...
  test("should trigger multiple workflow runs", async () => {
    ...
    await mockQStashServer({
        ...
        expect(result).toEqual([
          {
            ...
            workflowCreatedAt: 1735689600000,
            deduplicated: false,
          },
          {
      ...
      responseFields: {
        status: 200,
        body: [
          { workflowRunId: `wfr_${myWorkflowRunId}`, workflowCreatedAt: 1735689600000 },
          { workflowRunId: `wfr_${myWorkflowRunId2}`, workflowCreatedAt: 1735689600000 },
        ],
      },
      receivesRequest: {
        method: "POST",
  ...
  test("should fall back to the requested run id when the run is deduplicated", async () => {
    const myWorkflowRunId = `mock-${getWorkflowRunId()}`;
    await mockQStashServer({
      execute: async () => {
        const result = await client.trigger({
        ...
        // a deduplicated run isn't created, so the server returns neither an
        // id nor a creation time for it.
        expect(result).toEqual({
          workflowRunId: `wfr_${myWorkflowRunId}`,
          workflowCreatedAt: 0,
          deduplicated: true,
      ...
      responseFields: {
        status: 200,
        body: [{ workflowRunId: "", workflowCreatedAt: 0, deduplicated: true }],
      },
      receivesRequest: {
        method: "POST",
        url: `${MOCK_QSTASH_SERVER_URL}/v2/batch/trigger`,
